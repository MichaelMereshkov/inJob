...
//  inJob
//
//  Created by Михаил Мерешковв on 16.12.2023.
//

import UIKit

final class NewAdAddPhotoCell: UITableViewCell, TableCellConfigurable {
    ...
    func setup(viewModel: TableCellViewModelProtocol) {
        ...
        if viewModel.isLaik == true {
            iconImage.tintColor = .brandColor
        }
    ...
    func touchConfirmButton() {
        if iconImage.tintColor == .white {
            iconImage.tintColor = .brandColor
        } else if iconImage.tintColor == .brandColor {
            iconImage.tintColor = .white
        }
    }
